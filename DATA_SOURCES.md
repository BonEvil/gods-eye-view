...
Mapped ALPR locations come from OpenStreetMap, including contributions from the
[DeFlock community](https://deflock.org). Data is © OpenStreetMap contributors,
[ODbL 1.0](https://www.openstreetmap.org/copyright). No DeFlock service endorsement
or exact synchronization with its rendered map is implied.

The layer queries bounded regions for `man_made=surveillance` and
`surveillance:type=ALPR` via the existing cached Overpass proxy. It shows at most
2,000 records per region, requests a closer view when saturated, and does not
query global views. The proxy's existing disk cache can retain data for seven
days. The details panel exposes the OSM data snapshot, element edit time and
mapped check date where supplied; edits do not certify field verification.

The optional short direction arrow represents the mapped bearing only, not a
measured detection range. Locations are crowdsourced, coverage can be incomplete,
and operating status is unknown. This layer supplies no video or plate records.
